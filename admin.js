...
function applyFilters() {
  ...
  adminState.filtered = adminState.records.filter((record) => {
    const nameMatch = !searchValue || record.customer?.name?.toLowerCase().includes(searchValue);
...
function renderStats() {
  ...
  const uniqueCustomers = new Set(
    adminState.records.map((record) => `${record.customer?.name || ""}-${record.customer?.age || ""}-${record.customer?.gender || ""}`)
  ).size;
  ...
  statsGrid.innerHTML = `
    ...
    <article class="stat-card">
      ...
      <p class="stat-sub">고유 고객 ${uniqueCustomers}명 · 최근 ${latest ? latest.customer.name : "-"}</p>
    </article>
  `;
  ...
  responseList.innerHTML = adminState.filtered.map((record) => `
    <button class="response-card ${record.id === adminState.selectedId ? "is-active" : ""}" type="button" data-record-id="${record.id}">
      <div class="response-card-head">
        <p class="response-name">${record.customer.name}</p>
  ...
  detailView.innerHTML = `
    <div class="detail-block">
      <div class="detail-block-head">
        <h3>${record.customer.name} 고객 기본 정보</h3>
...
function deleteRecord(recordId) {
  ...
  const shouldDelete = window.confirm(`${record.customer.name} 응답을 삭제할까요? 이 작업은 되돌릴 수 없습니다.`);
  if (!shouldDelete) {
    return;
  }
