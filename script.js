...
function getCustomerName() {
  return state.answers.customerInfo?.name?.trim() || "";
}
...
function renderInfoForm() {
  ...
  form.innerHTML = `
    <div class="field-row">
      <div class="field-group">
        <label class="field-label" for="customer-name">이름</label>
        <input id="customer-name" class="field-input" type="text" placeholder="고객 이름" lang="ko" inputmode="text">
      </div>
      <div class="field-group">
  ...
  nameInput.addEventListener("input", (event) => {
    persistInfo({ name: event.target.value });
  });
...
function validateCurrentStep() {
  ...
  if (!hasAnswer) {
    errorMessage.textContent = question.type === "infoForm"
      ? "이름, 나이, 성별을 입력한 뒤 다음 단계로 이동해 주세요."
...
function buildSurveyRecord() {
  ...
  return {
    ...
    customer: {
      name: state.answers.customerInfo?.name?.trim() || "이름 없음",
...
function renderSummary() {
  ...
  resultHeadline.textContent = `${customerPrefix}${result.headline}`;
  resultSummary.textContent = customerPrefix ? `${customerPrefix}${result.summary}` : result.summary;
